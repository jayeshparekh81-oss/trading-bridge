...
describe("SEGMENT_MIN_CAPITAL constants", () => {
  it("holds the founder's stated minimums in rupees", () => {
    expect(SEGMENT_MIN_CAPITAL.cash).toBe(20_000);
    expect(SEGMENT_MIN_CAPITAL.options).toBe(100_000);
  ...
  it("formats via the app's existing formatCurrency helper (no bespoke formatter)", () => {
    expect(formatCurrency(SEGMENT_MIN_CAPITAL.cash, { compact: true })).toBe("₹20,000");
    expect(formatCurrency(SEGMENT_MIN_CAPITAL.options, { compact: true })).toBe("₹1.0L");
