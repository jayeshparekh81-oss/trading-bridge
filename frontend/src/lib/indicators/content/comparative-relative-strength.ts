...
export const COMPARATIVE_RELATIVE_STRENGTH: IndicatorContent = {
  ...
  indian_context:
    "Comparative Relative Strength is indispensable for Indian retail running sector-rotation strategies. NIFTY's sectoral indices (BANKNIFTY, NIFTY IT, NIFTY PHARMA, NIFTY AUTO, NIFTY METAL) show clear rotation patterns — typically 4-8 weeks per sector leading. The 2023 PSU bank rally, the 2024 defense stocks run, and the 2024 pharma comeback were all visible in sector CRS weeks before mainstream media coverage. For pair trading, common Indian pairs include HDFC/ICICI, INFY/TCS, M&M/Tata Motors — historically correlated with strong mean-reverting CRS dynamics. CRS is also useful for index futures decisions: when BANKNIFTY/NIFTY is rising, BANKNIFTY futures tend to outperform NIFTY futures on a leveraged basis.",
};
