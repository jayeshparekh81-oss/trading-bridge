...
export const ACCUMULATION_DISTRIBUTION: IndicatorContent = {
  ...
  indian_context:
    "A/D on NIFTY F&O cash equities works particularly well on RIL, HDFC Bank, ICICI Bank, and TCS where institutional volume dominates and the indicator's volume-flow read is meaningful. During FII outflow weeks, A/D often turns negative across large-caps even as price holds — a useful early-warning indicator. Avoid on index futures (NIFTY/BANKNIFTY) where hedging flows distort the signal. For mid-cap F&O like Tata Power, A/D signals during sector rotation (e.g., into renewables in 2023) preceded price by 2-4 weeks. Compare A/D across sector pairs (banking vs IT) for relative-strength rotation cues.",
};
