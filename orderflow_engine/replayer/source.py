...
class ReplaySource:
    ...
    def depth_files(self) -> list[Path]:
        """R1 depth instrument parquet files under ``depth/`` (events excluded),
        filtered + deterministically sorted by name. Empty if no depth subtree."""
        depth_dir = self.day_dir / "depth"
        if not depth_dir.is_dir():
            return []
        return sorted(p for p in depth_dir.glob("*.parquet")
                      if p.name != "events.parquet" and self._matches(p))
