...
class Consumer(Protocol):
    ...
    # R1: 20-level depth rows (side-tagged). A consumer that doesn't care about
    # depth can leave the default no-op below (it's Optional in the Protocol only
    # in spirit — concrete consumers should define it; the reference ones do).
...
class HashingConsumer:
    ...
    def on_depth(self, parsed: dict, ts_recv_ns: int) -> None:
        parts = ["D", str(ts_recv_ns)] + [_canon(parsed.get(c)) for c in DEPTH_COLUMNS]
        self._h.update(("\x1f".join(parts) + "\x1e").encode("utf-8"))
        self.count += 1
