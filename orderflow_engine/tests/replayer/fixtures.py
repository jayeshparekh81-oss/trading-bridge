...
from recorder.depth_schema import DEPTH_SCHEMA, SIDE_BID, normalize_depth_row
...
def write_depth(day_dir: Path, symbol: str, sid: int, rows: list[dict]) -> Path:
    """Write a depth instrument file under day_dir/depth/. rows: dicts with
    ts_recv_ns (+ optional side/msg_seq/price_1...)."""
    depth_dir = day_dir / "depth"
    depth_dir.mkdir(parents=True, exist_ok=True)
    norm = []
    for i, r in enumerate(rows):
        base = {"security_id": sid, "side": r.get("side", SIDE_BID),
                "seq_local": r.get("seq_local", i)}
        norm.append(normalize_depth_row({**base, **r}))
    tbl = pa.Table.from_pylist(norm, schema=DEPTH_SCHEMA)
    out = depth_dir / f"{symbol}_{sid}.parquet"
    pq.write_table(tbl, str(out))
    return out


def depth_row(ts_recv_ns: int, side: int = SIDE_BID, **extra) -> dict:
    d = {"ts_recv_ns": ts_recv_ns, "side": side}
    d.update(extra)
    return d
