"""CP5 (run 3) — (1) SYNTHETIC driftless random walk through the REAL engine (trailing sigma + joint object): recovered
...
STEP_S = 0.2; SESS_S = 6.25 * 3600; NSTEP = int(SESS_S / STEP_S); SIG_STEP = 0.3; N_SESS = 30; EV_PER_SESS = 60; TOL_PP = 3.0
...
for seed in (101, 202):
    ...
    for s in range(N_SESS):
        t_up, t_dn, censor, sigma = synth_session(rng, mu=mu); ok = np.flatnonzero(~np.isnan(censor) & ~np.isnan(sigma)); pick = np.sort(rng.choice(ok, size=EV_PER_SESS, replace=False))
    ...
    print(f"  INJECTED DRIFT seed {seed} (mu = +0.01 sigma_step per 200 ms step): worst |recovered - closed form| = {worst_d:.2f} pp -> {'PASS' if worst_d <= TOL_PP else 'FAIL'}")
