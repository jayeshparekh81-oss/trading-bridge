...
| 3 | ~~Gate (d): 3 branches not on main~~ → **2 branches still pending gate (d)** | ✅ VERIFIED | `feat/queue-ccc-historical-candles-skeleton` + `fix/queue-ddd-migration-027-uuid-cast` still on origin, not merged. **`feat/queue-eee-indicator-smoketests` MERGED to main 2026-06-13 via PR #13** (`origin/main` HEAD advanced `f62585d` → `34357dd`). EEE remote branch ref pending delete. |
