...
class Settings(BaseSettings):
    ...
    # ─── Market Strength Shield (W3.4 port, default OFF, EXIT DELAY) ────
    # See app.services.market_shield_service. Holds Pine's EXIT signal
    # for up to 2 closed 15-min bars (30 min) when 4-index breadth is
    # strong AND the position isn't already actively falling. ATR
    # override (loss >= 1×ATR) releases immediately. Cold-start (no
    # index OHLC in Redis) = pass-through. Phase 1: index data fetcher
    # is deferred — until a separate worker populates `mshield:idx:*`
    # keys, the shield stays in cold-start and is effectively inert.
    market_shield_enabled: bool = Field(
        default=False,
        description=(
            "Master toggle for the Market Strength Shield. When False "
            "(default), the shield is fully inert — no Redis writes, no "
            "compute, no hold. When True, EXIT signals are evaluated "
            "against 4-index breadth (NIFTY50, BANKNIFTY, SENSEX, BSE_MIDCAP) "
            "and held for max 2 bars when ≥3 of 4 indices are bullish "
            "AND the position isn't already actively falling (>=1×ATR "
            "loss). Releases on next Pine webhook arrival when 30 min "
            "elapsed OR drop crosses 1×ATR. RULE 1: never autonomously "
            "trades — only delays Pine's pre-existing EXIT directive. "
            "Default OFF until W3.4 paper-validated."
        ),
    )
