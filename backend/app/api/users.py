...
async def add_broker(
    ...
    # NOTE: Manual "Add Broker" flow ALWAYS uses the atomic relink helper
    # (deactivate old active cred + INSERT + repoint strategies). Explicit
    # user intent — no feature flag gate. Cron path (auto_login.py) is
    # gated on CRED_RELINK_ENABLED for staged rollout.
    ...
    try:
        result = await relink_strategies_to_new_credential(
            db,
            user_id=user.id,
            broker_name=broker_name_val,
            new_cred=cred,
    ...
    return {
        ...
        "deactivated_credential_ids": [str(x) for x in result.deactivated_credential_ids],
