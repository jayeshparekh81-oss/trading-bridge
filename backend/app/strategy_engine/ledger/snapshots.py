...
#: / ``operator_estimate`` / NULL never are.
#:
#: 🔴 ``operator_estimate`` was briefly a member (11 Sep) and was removed on
#: 16 Sep. A snapshot is append-only and hash-chained: a chain that folded a
#: human's estimate into ``cumulative_pnl_inr`` could never be annotated
#: afterwards. The founder's rule — every P&L from an actual fill, else NULL
#: with a reason — settles it. The ledger publishes fills or it publishes
#: nothing.
