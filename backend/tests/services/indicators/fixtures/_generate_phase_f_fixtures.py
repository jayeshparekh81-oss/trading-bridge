...
def regenerate_legacy_bb_expected() -> None:
    """Regenerate ``bb_expected.csv`` against POST-FIX TRADETRI output.

    The legacy ``bb_expected.csv`` (committed 2026-05-14, Day-6 sprint)
    was generated by TA-Lib + the now-removed sqrt(N/(N-1)) correction
    in ``bb.py:67-72``. After the Phase F authorized BB fix (commit
    ``63932b0``), TRADETRI's BB output equals Pine ``ta.bb`` to float
    epsilon, so the legacy fixture has to be refreshed.

    Reuses the existing :func:`shared_input.csv` (LCG-generated 200
    bars from :mod:`conftest`) as input so the existing
    ``test_bb.py::test_tradingview_result_match`` keeps the same input
    coverage. Only the expected values shift.
    """
    import sys
    ...
    shared_input = _FIX_DIR / "shared_input.csv"
    bb_expected = _FIX_DIR / "bb_expected.csv"
    candles = load_input_csv(shared_input)
    ...
    write_expected_csv(
        ...
        series={
            "upper": out["upper"].tolist(),
            "middle": out["middle"].tolist(),
            "lower": out["lower"].tolist(),
        },
    )
    print(
        f"Regenerated {bb_expected.relative_to(_FIX_DIR.parent.parent.parent)} "
        f"from {len(candles)} input bars (post-BB-fix Pine-equivalent output)."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--regen-legacy-bb",
        action="store_true",
        help="Also regenerate legacy bb_expected.csv from shared_input.csv "
             "(authorized override per PHASE_F_OVERRIDE_LOG.md).",
    )
    args = parser.parse_args()
    main()
    if args.regen_legacy_bb:
        regenerate_legacy_bb_expected()
