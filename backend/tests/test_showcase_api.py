...
class _FakeResult:
    ...
    def scalar_one_or_none(self):
        # showcase_live's third query is the published-listing lookup. None models
        # "no published listing", which is what these tests assert against.
        return None
...
def test_list_three_strategies_with_net_headline():
    ...
    assert set(strats) == {"s1", "s2", "s3"}
    assert strats["s1"]["live_status"]["track_type"] == "LIVE_REAL"
    assert strats["s2"]["live_status"]["track_type"] == "LIVE_NO_TRADES"
    assert strats["s3"]["live_status"]["track_type"] == "PAPER"
...
def test_list_serves_net_not_raw():
    res = asyncio.run(api.list_showcase())
    s1 = next(s for s in res["strategies"] if s["key"] == "s1")
    doc_s1 = next(s for s in _doc()["strategies"] if s["key"] == "s1")
    net_avg = doc_s1["backtest"]["net"]["aggregate"]["all"]["avg_pct_per_trade"]
    raw_avg = doc_s1["backtest"]["raw"]["aggregate"]["all"]["avg_pct_per_trade"]
    assert s1["headline_net"]["avg_pct_per_trade"] == net_avg
    assert s1["headline_net"]["avg_pct_per_trade"] != raw_avg  # NET, not RAW


# ── GET /api/showcase/{key} (detail) ────────────────────────────────────────
def test_detail_net_structure_and_slice_caveats():
    d = asyncio.run(api.showcase_detail("s1"))
    bt = d["backtest"]
    assert bt["basis"] == "net"
    ...
    doc_s1 = next(s for s in _doc()["strategies"] if s["key"] == "s1")
    assert bt["aggregate"]["all"] == doc_s1["backtest"]["net"]["aggregate"]["all"]
...
def test_live_zero_reconciled_is_tracking_active():
    res = asyncio.run(api.showcase_live("s1", session=_FakeSession(0)))
...
def test_live_paper_strategy_reports_no_live():
    res = asyncio.run(api.showcase_live("s3", session=_FakeSession(0)))
    assert res["status"] == "paper_no_live"
    assert res["reconciled_trades"] == 0


def test_live_never_fabricates_pnl_even_with_reconciled_count():
    res = asyncio.run(api.showcase_live("s1", session=_FakeSession(7)))
